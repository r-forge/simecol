...
\Abstract{
  ...
  This document is intended as a lose collection of sections that
  describe different aspects of modelling and model implementation in
  \proglang{R} with the \pkg{simecol} package.

  The howto-collection supplements the original publication of
  \citet{Petzoldt2007} from which an updated version is also part of
  this package. Please refer to that publication when citing this
  work.
...
(e.g. diferential equation solvers or interpolation routines) remain
...
\item You can have several independent versions of one model in the
  ...
  identifiers can be re-used and it is, for example, not neccessary to
...
working model object before starting to use simecol.
...
namely f, func, times, parms and init.  The implementation is similar
to the examples of package \pkg{deSolve} and we can solve it exactly
in the same manner:
...
declarative ``\code{=}'' for the slot definitions, that some of the
names (\code{y}, \code{func}) were changed to the pre-defined slot
names of \pkg{simecol} and that all the slot definitions are now comma
separated arguments of the \code{new} function that creates the
\code{upca} object. The solver method \code{lsoda} is also given as a
character string pointing to the original \code{lsoda} function in
package \pkg{deSolve}.
...
slots and an additional slot \code{out} holding the output values.  A
...
It is also possible to extract the results with the accessor function
\code{out}, and to use arbitrary, user-defined plot functions:
...
afterwards. In the following, we derive to new clones with default
parameter settings from the original \code{upca}-object, and then add
the Holling II functional resonse to scenario 1 and the Lotka-Volterra
functional response to scenario 2 (figure \ref{howto-upca9}):
...
\caption{\label{howto-upca9} Two scenarios of the UPCA model (left:
  functional response f is Holling II, right f is Lotka-Volterra).}
\end{figure}
...
but they appear as an additional burden during the development of
\pkg{simecol} objects, in particular if these models are technically
not that simple as in our example.

Fortunately, there is an easy workaround. This is, implementing the
...
the object is straigtforward, given that you keep the general
...
as a workspace function \code{fmain}\footnote{Note that this function
  must never be named ``func'', for some rather esotheric internal
...
workspace with only the neccessary objects.
...
%user-defined solvers and assigning a function to the solver-slot \dots
...
The `define' macros are a typical \proglang{C}-trick to get readable
names for the parameters. This method is simple and efficient, and
there are, of course, more elaborate possibilities, for example using
dynamic variables.
...
Compilation requires an installed \proglang{C} compiler (gcc) and some
other tools that are quite standard on Linux, and which are also
available for the Macintosh or, form of the R-Tools
collection\url{http://www.murdoch-sutherland.com/Rtools/} provided by
...
The answer is: Speed of computation is not the only thing, it is the
compromize between execution speed and programming effort. Programming
in scripting languages like \proglang{R} is much more convenient than
in compiled languages like \proglang{C} or \proglang{FORTRAN}, so
programming in compiled languages does only pay its effort required if
models are quite large or if very many model runs are required. And
even in such cases, a mixed approach \proglang{R} \textbf{and}
\proglang{C} can be more efficient, because it is only necessary to
implement the core functionality of the model in \proglang{R} and most
of data manipulation and scenario control can be done in \proglang{R}.
...
and but you \textbf{can} use \proglang{C} etc. whenever neccessary,
