...
\code{main} function of a \pkg{simecol}-object, but we do not want to
go this way here. What we do is to go the more efficient way, i.e. we
tell the solver \code{lsoda} where to find the model in the DLL.
