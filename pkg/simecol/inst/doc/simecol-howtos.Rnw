...
analysis and Markov chain Monte Carlo.  At time of writing the package
was still under development; a preliminary version is available from
...
simular to the above, including fine-tuning of the underlying
optimization algorithms. However, in contrast to \code{fitOdeModel},
\pkg{FME}'s \code{modeFit} is more powerful. It allows to use several
...
minimization criteria (cost functions) and the existence of summary
functions to extract the outputs.
