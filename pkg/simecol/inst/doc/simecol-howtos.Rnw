...
\pkg{FME} (Flexible Modeling Environment) is a package that contains
functions to perform complex applications of models, consisting of
differential equations, especially fitting models to data, sensitivity
analysis and Markov Chain Monte Carlo.  The package is still under
development; a preliminary version is available from
\url{http://fme.r-forge.r-project.org/}, future versions will be
available from CRAN (\url{http://cran.r-project.org/}).
...
observer data contain a \code{time} column. This is already available
...
<<echo=FALSE,quiet=TRUE,split=TRUE,eval=FALSE>>=
fme <- require(FME)
@
...
%%=======================================================================
\chapter{Troubleshoouting}

This chapter lists the reasons of the most common problems error
messages. You can xontribute to this sections by submitting bug
reports.
...
\item[Reasons:] The most commmon reason is to forget the required
...
zero-length variable name
  \end{verbatim}
\item[Solution:] Use ``='' instead of ``<-'' for lists (e.g. function
  definition in equations) and function arguments.
\end{description}
...
\section{Errors messages during model simulations}
...
\item[Reasons:] The number of state variables of ODE systems must be
  ...
  (e.g. \code{rk4}) and of other compatible solver packages may issue
  similar messages.
