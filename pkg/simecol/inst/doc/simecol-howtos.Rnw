...
\title{\pkg{simecol}-Howto: Tipps, Tricks and Building Blocks}
%\VignetteIndexEntry{simecol-Howto: Tipps, Tricks and Building Blocks}
...
\pkg{simecol}. It can be loaded like a data set; two working
copies \code{cs1} and \code{cs2} are made for further use:
...
variable time step algorithm (e.g. \code{lsoda} and to set external
...
one hand, scaling is necessary because they may be of different order
...
parameters are different or these two.
...
\pkg{FME} (Flexible Modeling Environment) is a package that
contains functions to perform complex applications
of models, consisting of differential equations, especially
fitting models to data, sensitivity analysis and Markov Chain Monte Carlo.

The package is still under development; a preliminary version is available from
\url{http://fme.r-forge.r-project.org/}, future versions will be available from
CRAN (\url{http://cran.r-project.org/}).

The essential principles of model fitting with \pkg{FME} are similar quite
simular to the above, including fine-tuning of the underlying optimization
algorithms. However, in contrast to \code{fitOdeModel}, \pkg{FME}'s \code{modeFit}
is more powerful. It allows to use several additional optimizers and gives more
detailed output, especially standard errors and covariances (correlations)
of parameter estimates. Other advantages are the flexible way to define own
minimization criteria (cost functions) and the existence of summary functions
to extract the outputs.

The following we give a full example to demonstrate its use with the same model
and data as above.

We use the chemostat model again, together with the test data set. The user
defined cost function (\code{Cost}) contains simulation and comparison between
simulated and observed data (function \code{modCost}) as last statement in the
function. Note that \code{modCost} requires that both simulated and observer
data contain a \code{time} column. This is already available in \code{ysim}
(returned by the solver), but we have to add it also to \code{yobs}. As above,
appropriate scaling (resp. weighting) of variables is also an important point.
Here, setting \code{weight="std"} does the same as the default \code{sd.yobs}
in the former example, other possibilities are described in the respective
online help pages.

%%enable the following by default, once FME is on CRAN

%% workaround to omit minpack.lm license here
